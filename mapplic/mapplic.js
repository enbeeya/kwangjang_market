...
(function($) {
	...
	var Mapplic = function() {
		var self = this;
		// 여기에서 map height를 정하면 된다.
		// json 에서 정한거는 쓸모가 없네... 먹지도 않고 ~~
		// 
		// map의 크기를 결정 하려면 
		//    height : mapplic.js에서 self.o.height 값 정의 함.
		//    width  : style.css의 .map-container 에서 정의 함. 
		self.o = {
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				// tooltip 상단에 'mapplic-tooltip-close' class로 'close' 버튼을 넣고 기능을 정의 한다.
				...
				// locations data에 link가 있는 경우 'mapplic-tooltip-link' class로 'More' 버튼을 넣어 준다.
				this.link = $('<a>link</a>').addClass('mapplic-tooltip-link').attr('href', '#').hide().appendTo(this.el);
		...
		function Minimap() {
			...
			this.init = function() {
				...
				// minimap height control하기
				this.el.css('height', 150);
		...
		function Sidebar() {
			...
			this.init = function() {
				...
				if (self.o.search) {
					...
					var input = $('<input>').attr({'type': 'text', 'spellcheck': 'false', 'placeholder': '상호 검색 ...'}).addClass('mapplic-search-input').keyup(function() {
		...
		var processData = function(data) {
			...
			// self.o.sidebar 를 false 처리하면 되지 않을까 ?
			// self.o.sidebar = false;
			// 음, 되는군 ... side bar가 없어지면서 전체 영역을 Map으로 사용하는군. 
			// 그런데 주어진 mapwidth, mapheight 크기대로 안되는 문제가 있음.
			// 스스로 다시 scaling 하는것 같은데 ~~ 왜그런가 ?
			//
			// self.o.height 에 높이값이 들어 있는데 맨 앞의 var Mapplic = function()에서 고정 시켜 놓았음.
			// 왜 고정 시켜 놓은건지 ?
			// width는 style.css의 .map-container 에서 정의 하고 있음. 
			// 그럴거면  json의 mapwidth, mapheight는 왜 정의 하는건지 ?			
			
			if (!self.o.sidebar) self.container.css('width', '100%');


			// 현재 값은
			// self.contentWidth=data.mapwidth=800
			// self.contentHeight = data.mapheight=600

			self.o.height = data.mapheight;
			
			self.contentWidth = data.mapwidth;
			self.contentHeight = data.mapheight;

			// 현재 주어진 값은 800,600 이므로
			// self.hw_ratio = 600 / 800 = 0.75
			self.hw_ratio = data.mapheight / data.mapwidth;
			
			// 이 항목이 map 크기를 container 크기에 맞추어 scale을 조정하고 있음.
			...
			// Create sidebar
			// 어떻게 하면  side bar가 안보이게 할까 ?  141020
			// self.o.sidebar 를 false 처리하면 되지 않을까 ?
			// 막으니까 side bar는 안보이는데 오른쪽이 비는군 ~~ 
			if (self.o.sidebar) {
			...
			// json 데이터를 읽어 각층 및 업체명 자료를 setting 
			if (data.levels) {
